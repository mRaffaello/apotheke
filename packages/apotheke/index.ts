...
function makeParser(pluginPath: string, parserName: string): PrettierParser {
    ...
    return {
        ...
        locStart(node: unknown): number {
            return (
                cached?.locStart ?? ((n: unknown) => (n as Record<string, unknown>).start as number)
            )(node);
        },
        locEnd(node: unknown): number {
            return (
                cached?.locEnd ?? ((n: unknown) => (n as Record<string, unknown>).end as number)
            )(node);
        },
        ...
        async preprocess(text: string, opts: { filepath?: string }): Promise<string> {
            ...
            const after = basePreprocess
                ? await (basePreprocess as (t: string, o: typeof opts) => Promise<string>)(
                      text,
                      opts
                  )
                : text;
